...
**Phase 0~5 완료.** 핵심 툴 표면이 구현·리뷰되었습니다:
`status`, `run`/`stop`, `scene`, `node`(읽기+쓰기), `signal`, `output`, `eval`,
`screenshot`, `batch` + `--json`/`--ids` 출력 모드. 명령 레퍼런스는
...
```
cmd/         Go CLI 명령 (status, run/stop, scene, node, signal, output, eval, screenshot, batch)
