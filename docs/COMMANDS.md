...
| `run [--scene <res://...>] [--current] [--wait]` | `run` | ☐ | Play the main scene (default), the current scene (`--current`), or a specific scene (`--scene`). `--wait` blocks until play starts. |
| `stop` | `run` | ☐ | Stop the running scene. |
