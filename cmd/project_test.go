...
func TestParseProjectArgs(t *testing.T) {
	tests := []struct {
		...
		{name: "reimport", args: []string{"reimport", "res://icon.svg"}, wantAction: "reimport"},
		...
		{name: "reimport missing path", args: []string{"reimport"}, wantErr: true},
		{name: "missing subcommand", wantErr: true},
...
func TestProjectActionMutates(t *testing.T) {
	tests := []struct {
		...
		{action: "scan", want: true},
		{action: "reimport", want: true},
		{action: nil},
	}
	for _, tt := range tests {
