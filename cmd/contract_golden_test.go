...
func contractCases() []contractCase {
	...
	return []contractCase{
		...
		{name: "status_capabilities", args: []string{"status", "--capabilities"}, responses: map[string]string{"status": "@status"}, golden: "status_capabilities"},
		{name: "capture_evidence", args: []string{"screenshot", "--evidence"}, responses: map[string]string{"status": `{"ok":true,"data":{"capabilities":{"linked_evidence":"supported"}}}`, "screenshot": `{"ok":true,"data":{"path":"fixture.png","width":640,"height":360,"evidence":{"available":true,"source":"editor_preview","editor_session_id":"fixture","captured_at_unix_ms":1,"atomic_snapshot":false}}}`}, golden: "capture_evidence"},
		{name: "save_failed_evidence", args: []string{"resource", "set", "res://value.tres", "--prop", "resource_name=after", "--evidence"}, responses: map[string]string{"status": `{"ok":true,"data":{"capabilities":{"linked_evidence":"supported"}}}`, "resource:set_evidence": `{"ok":false,"error":"save_failed: selected disk properties differ","data":{"effect":"applied","save_call":"succeeded","persistence":"failed","evidence":{"available":true}}}`}, wantExit: 1, wantStderrPrefix: "resource: save_failed:", golden: "save_failed_evidence"},
		{name: "save_old_addon_after_status", args: []string{"scene", "save", "--evidence"}, responses: map[string]string{"status": `{"ok":true,"data":{"capabilities":{"linked_evidence":"supported"}}}`, "scene:save_evidence": `{"ok":false,"error":"unknown scene action: save_evidence"}`}, wantExit: 1, wantStderrPrefix: "scene: unknown scene action:"},
		...
		{name: "game_ui_tree_ids", args: []string{"--ids", "game", "ui", "tree"}, responses: map[string]string{"game:ui_tree": `{"ok":true,"data":{"count":2,"controls":[{"name":"Start","path":"/root/Main/Start"},{"name":"Quit","path":"/root/Main/Quit"}],"truncated":false}}`}, golden: "game_ui_tree_ids"},
